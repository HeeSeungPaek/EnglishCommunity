...
public class BoardDao {
	...
	public List<BoardDto> getBoardList(int page, int limit) {
		...
		String board_list_sql = "select * from " + "(select rownum rnum, contentNumber, id, contentTitle, "
				+ "content, filename, filesize, refer, depth, step, " + "boardCode, boardName, "
				+ "TO_CHAR(ReportingDate, 'YYYY/MM/DD') AS ReportingDate from "
				+ "(select * from board order by Refer desc, Step asc)) " + "where rnum>=? and rnum<=?";
	...
	public BoardDto getDetail(int num) throws Exception {
		...
		try {
			String board_list_sql = "select * from " + "(select rownum rnum, contentNumber, id, contentTitle, "
					+ "content, filename, filesize, refer, depth, step, " + "boardCode, boardName, "
					+ "TO_CHAR(ReportingDate, 'YYYY/MM/DD') AS ReportingDate from "
					+ "(select * from board order by Refer desc, Step asc)) " + "where contentNumber=?";
...
//			}
			...
			sql = "insert into board (ContentNumber, Id, ContentTitle," + "Content, File, Refer, Depth, Step, "
					+ "ReadCount," + "notice, ReportingDate"
					+ "values(?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, sysdate)";

			String sql2 = "insert " +
			// "into replies(boardcode, boardname) values(?,?)" +
					"into board(ContentNumber, ContentTitle, Content, ReportingDate, "
					+ "Filename, filesize, Depth, Step, Refer, BoardCode, boardName, Id) " + "values(?,?,?,sysdate, "
					+ "?,?,?,?,?,?,?,?)";
			// "select * from dual";
	...
	public int boardReply(BoardDto board) {
		...
		try {
			...
			// update
			sql = "UPDATE board SET Step = Step + 1" + "WHERE Refer = ? AND Step > ?";
			...
			String sql2 = "insert " + "into board(ContentNumber, ContentTitle, Content, ReportingDate, "
	...
	public boolean boardDelete(int num, String userid, boolean userReplyCheck) throws SQLException {
		...
		// 제약조건(ORA-02292: integrity constraint (DBUSER.FK_BOARD_TO_REPLIES) violated -
		// child record found)
		// 특정 글번호의 comment(댓글)을 모두 삭제한 후에야 특정 글번호의 글(board)를 삭제할 수 있습니다
		String commentTable_delete_sql = "delete from commentTable where ContentNumber=?";
		String board_delete_sql = "delete from board where ContentNumber=?";
		...
		try {
			...
			/*
			 * //트랜잭션 conn = ds.getConnection(); conn.setAutoCommit(false);
			 * 
			 * pstmt = conn.prepareStatement(replies_delete_sql); pstmt.setInt(1, num);
			 * System.out.println("BoardDao/boardDelete/num: " + num); result =
			 * pstmt.executeUpdate();
			 * System.out.println("BoardDao/boardDelete/result: "+result);
			 * 
			 * pstmt2 = conn.prepareStatement(board_delete_sql); pstmt2.setInt(1, num);
			 * pstmt2.executeQuery(); conn.commit(); return true; /////////
			 */
		} catch (Exception e) {
	...
	public void setReadCountUpdate(int num) throws Exception {
		String sql = "update board set ReadCount = " + "ReadCount + 1 where ContentNumber = " + num;
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.executeUpdate();
	...
	public boolean isBoardWriter(int num, String id) {
		...
		String board_sql = "select * from board where contentnumber=?";
	...
	public boolean isRepliesWriter(int num, String id) {
		...
		String reply_sql = "select * from commentTable where contentnumber=?";
	...
	public int insertComment(CommentTableDto dto) {
		...
		try {
			...
			String query = "insert into commentTable" + "(commentNumber, commentContent, contentNumber, id, redate)"
	...
	public int getCommentsCount(int num) {
		...
		try {
			...
			String query = "SELECT CONTENTNUMBER, COMMENTNUMBER, COMMENTCONTENT, ID, REDATE    "
					+ "FROM COMMENTTABLE    " + "WHERE CONTENTNUMBER = ?";
	...
	public List<CommentTableDto> getPagedComments(int currpage, int pagesize, int num) {
		...
		String query = "SELECT * FROM"
				...
				+ "        FROM COMMENTTABLE)) " + " WHERE RN >= ? and RN <= ? CONTENTNUMBER = ?";
		...
		String query2 = "SELECT RN, COMMENTNUMBER, COMMENTCONTENT, CONTENTNUMBER, ID, REDATE FROM "
				...
				+ "WHERE CONTENTNUMBER = ? ORDER BY COMMENTNUMBER) " + "WHERE RN >= ? AND RN <= ?";
